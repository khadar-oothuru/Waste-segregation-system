  ...
  return (
    <div className="min-h-screen py-8 flex justify-center">
      <div className="w-full max-w-4xl bg-transparent p-8 rounded-lg shadow-lg lg:w-3/4 xl:w-2/3">
        ...
        <p className="text-2xl font-bold text-purple-700 mb-4">Total Waste Collected: <span className="text-purple-700">{totalWaste}</span></p>
        <p className="text-xl font-semibold text-blue-600 flex items-center mb-4">
          Dry Waste: <span className="font-bold text-blue-600">{dryWaste}</span>
        </p>
        <p className="text-xl font-semibold text-green-600 flex items-center mb-6">
          Wet Waste: <span className="font-bold text-green-600">{wetWaste}</span>
        </p>
          ...
          {records.slice().reverse().map((record, index) => (
            ...
            >
              ...
              <p className="text-xl text-blue-600 flex items-center mb-3">
                <FaRecycle className="mr-3 text-blue-500 text-2xl" /> Dry Waste: <span className="font-bold text-blue-800">{record.dryWaste}</span>
              </p>
              <p className="text-xl text-green-600 flex items-center mb-3">
                <FaWater className="mr-3 text-green-500 text-2xl" /> Wet Waste: <span className="font-bold text-green-800">{record.wetWaste}</span>
              </p>
              <p className="text-lg text-blue-500 flex items-center mb-3">
                <WiHumidity className="mr-3 text-blue-400 text-2xl" /> Humidity: <span className="font-bold text-blue-700">{record.humidity}%</span>
              </p>
              <p className="text-lg text-gray-600 flex items-center mb-3">
                <FaRulerVertical className="mr-3 text-gray-500 text-2xl" /> Distance: <span className="font-bold">{record.distance} cm</span>
              </p>
              <p className="text-sm text-gray-600 flex items-center">
                <FaClock className="mr-3 text-gray-500 text-2xl" /> Timestamp: <span className="font-bold">{record.timestamp ? format(new Date(record.timestamp), 'PPpp') : 'No timestamp available'}</span>
              </p>
            </li>
          ))}
        </ul>
